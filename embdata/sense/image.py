...
from typing import Any, ClassVar, List, SupportsBytes, Tuple, TypeAlias
...
from embdata.utils.image_utils import dispatch_arg, load_url, pil_to_data
...
class Image(Sample):
    ...
    pil: SkipJsonSchema[InstanceOf[PILImage]] | None = Field(
        default=None,
        repr=False,
        exclude=True,
    ...
    @cached_property
    def base64(self) -> str:
        """Returns the base64 encoded string of the image without unnecessary re-encoding."""
        if self.path:
            with Path(self.path).open("rb") as f:
                image_bytes = f.read()
            return base64lib.b64encode(image_bytes).decode("utf-8")

        buffer = io.BytesIO()

        if self.pil.mode != self.mode:
            self.pil = self.pil.convert(self.mode)

        self.pil.save(buffer, format=self.encoding.upper())
    ...
    def bytes(self) -> bytes:
        """Returns raw bytes of the image without unnecessary re-encoding."""
        if self.path:
            with Path(self.path).open("rb") as f:

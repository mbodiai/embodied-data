...
def rpy_to_rotation_matrix(rpy: np.ndarray, sequence: str = "zyx") -> np.ndarray:
...
def axis_angle_to_euler_scipy(axis_angle_vector: np.ndarray, sequence: str = "xyz") -> np.ndarray:
...
def euler_to_axis_angle_scipy(euler_angles: np.ndarray, sequence: str = "xyz") -> np.ndarray:
...
def align_vector_with_reference(vector: np.ndarray, reference: np.ndarray) -> np.ndarray:
    ...
    if vector_norm == 0 or reference_norm == 0:
        msg = "Input vectors must have non-zero norms."
        raise ValueError(msg)
    ...
    # Compute angles
    cos_theta = np.dot(vector_unit, reference_unit)
    cos_theta = np.clip(cos_theta, -1.0, 1.0)  # Clip to handle numerical errors
    angle = np.arccos(cos_theta)

    cos_theta_neg = np.dot(-vector_unit, reference_unit)
    cos_theta_neg = np.clip(cos_theta_neg, -1.0, 1.0)
    angle_neg = np.arccos(cos_theta_neg)

    # Convert angles to degrees for readability (optional)
    np.degrees(angle)
    np.degrees(angle_neg)

    # Return the vector (or its negative) that results in the smaller angle
    if angle_neg < angle:
        return -vector
    return vector
